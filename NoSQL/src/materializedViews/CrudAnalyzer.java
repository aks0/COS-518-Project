...
/**
 * GSP client for analyzing tables and columns accessed from provided SQL
 * queries
 */
public class CrudAnalyzer {
	...
	/**
	 * Analyze the provided SQL query String for information on accessed tables
	 * and columns in CRUD
	 * 
	 * @param query
	 * @return String representation of analysis in the form of: statement type
	...
	public String analyze(String query) {
		...
		if (ret == 0) {
			// iterate over every statement found to fill in tables and columns
			// arraylists
	...
	/**
	 * Uses statement object stmt to fill in StringBuilder result with lines in
	 * the form of: statement type tableName.columnName ...
	 * 
	 * @param stmt
	 * @param result
	 */
	private void analyzeStmt(TCustomSqlStatement stmt, StringBuilder result) {
		for (int i = 0; i < stmt.tables.size(); i++) {
			if (stmt.tables.getTable(i).isBaseTable()) {
				...
				if (operation.equalsIgnoreCase("deleted")
						|| operation.equalsIgnoreCase("inserted")) {
					continue;
				}
				...
				for (int j = 0; j < stmt.tables.getTable(i)
						.getObjectNameReferences().size(); j++) {
					// append table name
					result.append(stmt.tables.getTable(i).getFullName());
					result.append(".");
					// append column name
					result.append(stmt.tables.getTable(i)
							.getObjectNameReferences().getObjectName(j)
							.getColumnNameOnly());
					result.append("\n");
				}
			}
		...
		if (stmt.getWhereClause() != null) {
			result.append(new WhereCondition(stmt.getWhereClause()
					.getCondition()).getColumnsString());
		}
