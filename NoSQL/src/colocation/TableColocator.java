...
public class TableColocator {
    ...
    public static void colocate(ArrayList<Table> tables, List<Query> queries, MemorySize avgServerSize, int maxServers) {
            ...
            if (!entityMap.get(entityCenter).onlyContainsCenter()) {
                for (Table entity : entityMap.get(entityCenter)) {
                    if (!entity.equals(entityCenter)) {
                        replicationCandidates.add(entity);
                    }
                }
            }
        }
        ...
        ArrayList<Pair<Table, ServerGroup>> pairs = graph.getHighestReplicationScorePairs(10, replicationCandidates, serverGroups);
        ...
        for (Pair<Table, ServerGroup> pair : pairs) {
        	...
        	boolean replicated = group.addReplicatedTable(replicatedTable, 0, maxServers);
        	if (replicated) {
