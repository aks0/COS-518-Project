...
public class DemoClient {
	...
	public static void main(String[] args) {
		...
		// QueryLib.getQueryList("query_logs/query_workload500.sql");
		HashSet<TableSubset> subsets = TableSubsetProducer.produce(queryList,
				baseline, maxSize);
		...
		for (Query query : queryList) {
			...
			TableSubset bestSubset = CostEstimator.findBestSubset(query,
					candidateSubsets);
			selectedSubsets.add(bestSubset);
			System.out.println("\nBest column combination for query " + i++
					+ ": " + bestSubset);
			System.out.println("Cost : "
					+ CostEstimator.denormalizedCost(query, bestSubset));
			System.out.println("\n\n\n");
		}
	}
}
