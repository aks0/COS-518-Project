...
public class QueryGenerator {
	...
	private final static String qInnerTempl = "SELECT %s \nFROM %s \nWHERE %s = \"test\"";
	private final static String qOuterTempl = "SELECT %s \nFROM %s \nWHERE %s IN \n(%s)";
	...
	public static String genOuterQuery(List<Table> tables) {
		...
		Column where_column = inner_select_col.isForeignReference() ? inner_select_col
				.getForeignKeyReference() : inner_select_col;
		...
		str_select_columns = str_select_columns.substring(0,
				str_select_columns.length() - 2);
		...
		str_join_tables = str_join_tables.substring(0,
				str_join_tables.length() - 2);
		...
		// add to the query
		String query = String.format(qOuterTempl, str_select_columns,
				str_join_tables, str_where_column, inner_query);
		return query;
	}
	...
	public static Pair<String, Column> genInnerQuery(List<Table> tables) {
		...
		String query = String.format(qInnerTempl, table.getName() + "."
				+ select_column.getName(), table.getName(), table.getName()
				+ "." + where_column.getName());
	...
	public static void main(String[] args) throws IOException {
		...
		PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(
				"./query_logs/query_workload" + workload_size + ".sql")));
