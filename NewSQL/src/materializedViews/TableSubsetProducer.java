...
public class TableSubsetProducer {
	...
	/**
	 * Given queries, baseline threshold, and maxSize, produces candidate list
	 * of table subsets
	 * 
	 * @param queries
	 * @param baseline
	 ...
	 * @return
	 */
	public static HashSet<TableSubset> produce(ArrayList<Query> queries,
			double baseline, int maxSize) {
		...
		while (subsetSize <= maxSize && candidateSets.get(i - 1).size() > 0) {
			...
			for (Query query : queries) {
				TableSubset querySubset = new TableSubset(
						query.getReferencedColumns());
				ArrayList<TableSubset> sizedCombos = combos(
						querySubset.getColumns(), subsetSize);
					...
					if (!newCandidateSet.contains(subset)) {
						...
						for (TableSubset smallerSubset : smallerSubsets) {
							if (oldCandidateSet.contains(smallerSubset)
									&& CostEstimator.normalizedCost(subset,
											queries) >= baseline) {
	 ...
	 * @return
	 */
	private static ArrayList<TableSubset> subsetOneSizeSmaller(
			TableSubset subset) {
	 ...
	 * @return
	 */
	private static ArrayList<TableSubset> combos(NavigableSet<Column> columns,
			int size) {
		ArrayList<TableSubset> allSubsets = Util518.newArrayList();
		...
		int i = 0;
		for (Iterator<Column> iter = columns.iterator(); i < (columns.size()
				- size + 1)
				&& iter.hasNext(); i++) {
			Column pivot_col = iter.next();
			ArrayList<TableSubset> subsets = combos(
					columns.tailSet(pivot_col, false), size - 1);
